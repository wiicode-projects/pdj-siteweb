...
export function Testimonials() {
  const { t } = useLanguage();
  const { statsEnabled, testimonialsEnabled, testimonials, stats, showSection, resolveImage } = useWebsiteContent();
  ...
  const items = testimonialsEnabled ? testimonials : [];
  ...
  return (
    <section id="testimonials" className="py-24 bg-transparent relative overflow-hidden">
      <SectionContainer className="relative z-10">
        ...
        {testimonialsEnabled && items.length > 0 && (
          <>
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                          {resolveImage(item.imageUrl) && (
                            <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-primary/25 group-hover:border-primary/50 transition-colors shadow-md">
                              <img src={resolveImage(item.imageUrl)!} alt={item.name} className="w-full h-full object-cover" loading="lazy" />
                            </div>
                          )}
                          <div>
